...
September 9: Onboarding no longer requires a portrait checkbox or saving a recovery key. A plain note explains portrait visibility. The host guest book at `/host` can search registered guests and issue a three-word, ten-minute, single-use return phrase. Its Cloudflare Access email authentication is **pending Zero Trust Free activation and Access application configuration**; the deployed host APIs fail closed until configured. See [recovery and reveal handoff](handoff-2026-09-09-recovery-reveals.md).
